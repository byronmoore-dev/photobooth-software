...
Run `dist\Camera-Booth-Setup-0.19.1.exe`. The installer includes the x64 .NET camera bridge, Canon runtime DLLs, and the session-video encoder, so an event operator does not install either SDK separately.
